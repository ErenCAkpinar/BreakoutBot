...
PREV_CUT = pd.Timestamp("2026-08-24", tz="UTC")       # end of every earlier round's data
...
def always_on(bp: basis.BasisPanel, names: list[str]) -> np.ndarray:
    """Equal weight over `names` wherever they have data; no selection, no rebalance."""
...
def sl_stats(r: np.ndarray, bp: basis.BasisPanel, w: np.ndarray, sl: slice) -> dict:
    st = engine.stats(r[sl], TF)
    pt = basis.parts(bp, w, sl, PER_YEAR)
...
def main() -> None:
    ...
    # every config once over the whole panel — selection metrics read only the train slice
    grid_r, grid_w = {}, {}
    for cfg in GRID:
    ...
    ctrl_r = {k: basis.evaluate(bp, w) for k, w in controls.items()}

    rows = []
    oos_r, oos_w = [], np.zeros_like(bp.spot)
    for y in TEST_YEARS:
        t0 = int(np.searchsorted(bp.index, pd.Timestamp(f"{y}-01-01", tz="UTC")))
        t1 = int(np.searchsorted(bp.index, pd.Timestamp(f"{y + 1}-01-01", tz="UTC")))
        t1 = min(t1, n)
        train, test = slice(0, t0), slice(t0, t1)
        best = max(GRID, key=lambda c: engine.stats(grid_r[c][train], TF)["sharpe"])
        r = grid_r[best]
        oos_r.append(r[test])
        oos_w[test] = grid_w[best][test]
        st = sl_stats(r, bp, grid_w[best], test)
        st_in = engine.stats(r[train], TF)
        rows.append({"test yılı": y, "seçilen (top,lb,reb,minF)": f"{best[0]},{best[1]},{best[2]},{best[3]*100:.3f}%",
                     "eğitim SR": st_in["sharpe"], "eğitim yıllık %": st_in["ann_ret"] * 100, **{f"OOS {k}": v for k, v in st.items()},
                     **{f"{k} net %": sl_stats(ctrl_r[k], bp, controls[k], test)["net_%"] for k in controls}})
    wf = pd.DataFrame(rows)

    # concatenated OOS series (2022-01 → data end)
    t0 = int(np.searchsorted(bp.index, pd.Timestamp("2022-01-01", tz="UTC")))
    r_oos = np.concatenate(oos_r)
    summary = {"walk-forward OOS 2022→": {**engine.stats(r_oos, TF), **basis.parts(bp, oos_w, slice(t0, n), PER_YEAR)}}
    for k in controls:
        summary[f"{k} 2022→"] = {**engine.stats(ctrl_r[k][t0:n], TF), **basis.parts(bp, controls[k], slice(t0, n), PER_YEAR)}
    # in-sample reference: best Sharpe on ALL data, scored on the same 2022→ slice (what Tur 7 did)
    best_all = max(GRID, key=lambda c: engine.stats(grid_r[c], TF)["sharpe"])
    summary["in-sample en iyi SR (tüm veri seçimi) 2022→"] = {**engine.stats(grid_r[best_all][t0:n], TF),
                                                             **basis.parts(bp, grid_w[best_all], slice(t0, n), PER_YEAR),
                                                             "cfg": best_all}
    summ = pd.DataFrame([{"seri": k, "yıllık %": v["ann_ret"] * 100, "vol %": v["ann_vol"] * 100, "DD %": v["max_dd"] * 100,
                          "SR": v["sharpe"], "funding %/y": v["funding"], "basis %/y": v["basis_drift"], "maliyet %/y": v["cost"],
                          "net %/y": v["net"], "cfg": str(v.get("cfg", ""))} for k, v in summary.items()])

    # time-OOS: unseen days since the previous cut
    c0 = int(np.searchsorted(bp.index, PREV_CUT))
    days = (bp.index[-1] - bp.index[c0]).total_seconds() / 86400
    trows = []
    ...
    for label, names in (("BTC", ["BTCUSDT"]), ("ETH", ["ETHUSDT"]), ("likit-6 ort", LIQUID6), ("23 medyan", bp.symbols)):
        cols = [bp.symbols.index(s) for s in names]
    ...
    out(tos, f"C. Zaman-OOS: {bp.index[c0]:%Y-%m-%d} → {bp.index[-1]:%Y-%m-%d} (önceki turların görmediği günler), yıllıklandırılmış")
